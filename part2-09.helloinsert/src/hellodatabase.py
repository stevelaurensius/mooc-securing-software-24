...
def add_agent(conn, aid, name):
	name_var = (aid, name)
	conn.execute("INSERT INTO Agent VALUES (?,?)", name_var)
	conn.commit()
...
def delete_agent(conn, aid):
	name_var = (aid, )
	conn.execute("DELETE FROM Agent WHERE id=?", name_var)
	conn.commit()
	pass # write code here, don't forget to commit results once you execute the insert
...
def read_database(conn):
	...
	# write code here
	statement = 'SELECT id, name FROM Agent'
	agents = conn.execute(statement).fetchall()
	agents.sort()
